...
public class ArrayMath {
    ...
    public static void pairwiseScaleAddInPlace(double[] to, double[] from,
                                               double fromScale) {
        if (to.length != from.length) {
            throw new RuntimeException();
        }
    ...
    public static double[] pairwiseScaleAdd(double[] a, double[] b,
                                            double bScale) {
        double[] result = new double[a.length];
        for (int i = 0; i < a.length; i++) {
    ...
    public static double[] pairwiseMultiply(double[] a, double[] b) {
        if (a.length != b.length) {
            throw new RuntimeException(
                    "Can't pairwise multiple different lengths: a.length="
    ...
    /**
     * @return the index of the max value; if max is a tie, returns the last
     * one.
     */
    ...
    /**
     * Returns the largest value in a vector of doubles. Any values which are
     * NaN or infinite are ignored. If the vector is empty, 0.0 is returned.
     */
    ...
    /**
     * Returns the log of the portion between <code>fromIndex</code>, inclusive,
     * and <code>toIndex</code>, exclusive, of an array of numbers, which are
     * themselves input in log form. This is all natural logarithms. Reasonable
     * care is taken to do this as efficiently as possible (under the assumption
     * that the numbers might differ greatly in magnitude), with high accuracy,
     * and without numerical overflow. Throws an
     * {@link IllegalArgumentException} if <code>logInputs</code> is of length
     * zero. Otherwise, returns Double.NEGATIVE_INFINITY if
    ...
    public static double logSum(double[] logInputs, int fromIndex, int toIndex) {
        ...
        if (fromIndex >= 0 && toIndex < logInputs.length
                && fromIndex >= toIndex)
    ...
    public static double logSum(List<Double> logInputs, int fromIndex,
                                int toIndex) {
     ...
     * is 0.0 or NaN, throws an RuntimeException.
     */
    ...
    /**
     * Standardize values in this array, i.e., subtract the mean and divide by
     * the standard deviation. If standard deviation is 0.0, throws an
     * RuntimeException.
     */
    ...
    /**
     * Makes the values in this array sum to 1.0. Does it in place. If the total
     * is 0.0, throws a RuntimeException. If the total is
     * Double.NEGATIVE_INFINITY, then it replaces the array with a normalized
     * uniform distribution. CDM: This last bit is weird! Do we really want
     * that?
     */
    ...
    public static int sampleFromDistribution(double[] d, Random random) {
        ...
        // now compare its value to cumulative values to find what interval it
        // falls in
        double total = 0;
        ...
        return d.length - 1; // in case the "double-math" didn't total to
        // exactly 1.0
    }
    ...
    /**
     * Returns the Jensen Shannon divergence (information radius) between a and
     * b, defined as the average of the kl divergences from a to b and from b to
     * a.
     */
    ...
    /**
     * Fills the array with sample from 0 to numArgClasses-1 without
     * replacement.
     */
    ...
    public static void shuffle(int[] a, Random rand) {
        for (int i = a.length - 1; i >= 1; i--) {
            int j = rand.nextInt(i + 1); // a random index from 0 to i
            // inclusive, may shuffle with
            // itself
            int tmp = a[i];
            a[i] = a[j];
            a[j] = tmp;
     ...
     * pearsonCorrelation, it is perhaps more advisable to first standardize x
     * and y, then compute innerProduct(x,y)/(x.length-1).
     */
     ...
     * default value of 1000 iterations. See documentation for other version of
     * method.
     */
    ...
    /**
     * Takes a pair of arrays, A and B, which represent corresponding outcomes
     * of a pair of random variables: say, results for two different classifiers
     * on a sequence of inputs. Returns the estimated probability that the
     * difference between the means of A and B is not significant, that is, the
     * significance level. This is computed by "approximate randomization". The
     * test statistic is the absolute difference between the means of the two
     * arrays. A randomized test statistic is computed the same way after
     * initially randomizing the arrays by swapping each pair of elements with
     * 50% probability. For the given number of iterations, we generate a
     * randomized test statistic and compare it to the actual test statistic.
     * The return value is the proportion of iterations in which a randomized
    ...
    private static double absDiffOfMeans(double[] A, double[] B,
                                         boolean randomize) {
    ...
    /**
     * public static String toString(int[][] counts) { return toString(counts,
     * null, null, 10, 10, NumberFormat.getInstance(), false); }
     * <p/>
     * public static String toString(double[][] counts) { return
     * toString(counts, 10, null, null, NumberFormat.getInstance(), false); }
     * <p/>
     * public static String toString(float[][] counts) { return toString(counts,
     * 10, null, null, NumberFormat.getIntegerInstance(), false); }
     */
    ...
    public static double[][] covariance(double[][] data) {
        ...
        for (int i = 0; i < data[0].length; i++) {
            for (int j = 0; j < means.length; j++) {
                for (int k = 0; k < means.length; k++) {
                    covariance[j][k] += (means[j] - data[j][i])
                            * (means[k] - data[k][i]);
                }
            }
        }
        ...
        for (int i = 0; i < covariance.length; i++) {
            for (int j = 0; j < covariance[i].length; j++) {
                covariance[i][j] = Math.sqrt(covariance[i][j])
                        / (data[0].length);
            }
        }
        return covariance;
    ...
    /**
     * Simulate Arrays.copyOf method provided by Java 6 When/if the JavaNLP-core
     * code base moves past Java 5, this method can be removed
     *
     * @param original
     * @param newSize
     */
