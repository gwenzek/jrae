...
public class QNMinimizer implements Minimizer<DifferentiableFunction> {
	...
	private static final double p66 = 0.66; // used to check getting more than
											// 2/3 of width improvement
	 ...
	 * convergence, and to (attempt to) ensure numerical errors are not an
	 * issue. It can also be used for plotting the results of the optimization
	 * routine.
	 * 
	 * @author akleeman
	 * 
	...
	public class Record {
		...
		// before its been initialized you dont get a NAN but you will also
		// never
		...
		private int maxSize = 100; // This will control the number of func
									// values /
		// gradients to retain.
		...
		public Record(Function monitor) {
			this.mon = monitor;
		}

		public Record(Function monitor, double tolerance) {
			this.mon = monitor;
			this.TOL = tolerance;
		}

		public Record(Function monitor, double tolerance, PrintWriter output) {
			this.mon = monitor;
			this.TOL = tolerance;
			this.outputFile = output;
		}

		public Record(Function monitor, double tolerance, double eps) {
			this.mon = monitor;
			this.TOL = tolerance;
			this.EPS = eps;
		}
		...
		/*
		 * Initialize the class, this starts the timer, and initiates the
		 * gradient norm for use with convergence.
		 */
		...
		private void writeToFile(double fevals, double val, double gNorm,
				double time) {
			if (outputFile != null) {
				outputFile.println(fevals + "," + val + "," + gNorm + ","
						+ time);
			}
		}
		...
		/**
		 * This function checks for convergence through first order optimality,
		 * numerical convergence (i.e., zero numerical gradient), and also by
		 * checking the average improvement.
		 * 
		 * @return A value of the enumeration type
		 *         <p>
		 *         eState
		 *         </p>
		 *         which tells the state of the optimization routine indicating
		 *         whether the routine should terminate, and if so why.
		 */
		public eState toContinue() {
			...
			double previousVal = (size >= 10 ? values.get(size - 10) : values
					.get(0));
			double averageImprovement = (previousVal - newestVal) / size;

			// This is used to be able to reproduce results that were trained on
			// the
			// QNMinimizer before
			...
			if (useNumericalZero) {
				// This checks if the gradient is sufficiently small compared to
				// x that
				// it is treated as zero.
				if (gNormLast < EPS * Math.max(1.0, ArrayMath.norm_1(xLast))) {
					// |g| < |x|_1
					// First we do the one norm, because that's easiest, and
					// always bigger.
	...
	public class QNInfo {
		...
		public void setHistory(List<double[]> sList, List<double[]> yList) {
			...
			for (int i = 0; i < size; i++) {
				update(sList.get(i), yList.get(i), ArrayMath.innerProduct(yList
						.get(i), yList.get(i)), ArrayMath.innerProduct(sList
						.get(i), yList.get(i)), 0, 1.0);
			}
		}
		...
		/*
		 * The update function is used to update the hessian approximation used
		 * by the quasi newtion optimization routine.
		 * 
		 * If everything has behaved nicely, this involves deciding on a new
		 * initial hessian through scaling or diagonal update, and then storing
		 * of the secant pairs s = x - previousX and y = grad - previousGrad.
		 * 
		 * Things can go wrong, if any non convex behavior is detected (s^T y <
		 * 0) or numerical errors are likely the update is skipped.
		 */
		public int update(double[] newX, double[] x, double[] newGrad,
			...
			// allocate arrays for new s,y pairs (or replace if the list is
			// already
			// full)
			...
			// Here we construct the new pairs, and check for positive
			// definiteness.
			sy = 0;
			yy = 0;
			sg = 0;
       ...
       */
		...
		public int update(double[] newS, double[] newY, double yy, double sy,
			...
			try {
				...
				switch (scaleOpt) {
				/*
				 * SCALAR: The standard L-BFGS initial approximation which is
				 * just a scaled identity.
				 */
				case SCALAR:
					gamma = sy / yy;
					break;
				 ...
				 * approximation. The updating method used is used thanks to
				 * Andrew Bradley of the ICME dept.
				 */
				case DIAGONAL:

					double sDs;
					// Gamma is designed to scale such that a step length of one
					// is
					// generally accepted.
					gamma = sy / (step * (sy - sg));
					sDs = 0.0;
					...
					for (int i = 0; i < d.length; i++) {
						d[i] = (1 - d[i] * newS[i] * newS[i] / sDs) * d[i]
								+ newY[i] * newY[i] / sy;
					}
					...
					// If things have gone bad, just fill with the SCALAR
					// approx.
					if (minD <= 0 || Double.isInfinite(maxD)
							|| maxD / minD > 1e12) {
						System.err
				...
				// If s is already of size mem, remove the oldest vector and
				// free it up.
				...
				// NOTE: if applying QNMinimizer to a non convex problem, we
				// would still
	 ...
	 * computeDir()
	 * 
	 * This function will calculate an approximation of the inverse hessian
	 * based off the seen s,y vector pairs. This particular approximation uses
	 * the BFGS update.
	 */
	...
	public double[] minimize(DifferentiableFunction function,
			double functionTolerance, double[] initial) {
		return minimize(function, functionTolerance, initial, -1);
	}

	public double[] minimize(DifferentiableFunction dfunction,
			double functionTolerance, double[] initial,
	...
	public double[] minimize(DifferentiableFunction dfunction,
			double functionTolerance, double[] initial,
			int maxFunctionEvaluations, QNInfo qn) {

		say("QNMinimizer called on double function of " + dfunction.dimension()
				+ " variables,");
		if (mem > 0) {
			sayln(" using M = " + mem + ".");
		} else {
		...
		// initialize function value and gradient (gradient is stored in grad
		// inside
		// evaluateFunction)
		...
		if (outputToFile) {
			try {
				String baseName = "QN_m" + mem + "_" + lsOpt.toString() + "_"
						+ scaleOpt.toString();
				outFile = new PrintWriter(new FileOutputStream(baseName
						+ ".output"), true);
				infoFile = new PrintWriter(new FileOutputStream(baseName
						+ ".info"), true);
				infoFile.println(dfunction.dimension() + "; DomainDimension ");
				infoFile.println(mem + "; memory");
			} catch (IOException e) {
				System.err
						.println("Caught IOException outputing QN data to file: "
								+ e.getMessage());
				System.exit(1);
			}
		}
		...
		do {
			...
			try {
				sayln();
				// doEval = (its > 0 && evaluateIters > 0 && its % evaluateIters
				// == 0);
				// if (doEval) {
				// doEvaluation(x);
				// }
				...
				qn.rho.remove(0);
				qn.mem = qn.s.size();
				System.err.println("Caught OutOfMemoryError, changing m = "
						+ qn.mem);
			}
		...
		sayln("Total time spent in optimization: "
				+ nfsec.format(completionTime) + "s");
	 ...
	 * conditions.
	 */
	private double[] lineSearchBacktrack(Function func, double[] dir,
			double[] x, double[] newX, double[] grad, double lastValue)
		...
		// for first few steps, we have less confidence in our initial step-size
		// a
	...
	private double[] lineSearchMinPack(DifferentiableFunction dfunc,
			double[] dir, double[] x, double[] newX, double[] grad, double f0,
			double tol) throws MaxEvaluationsExceeded {
		double xtrapf = 4.0;
		int info = 0;
		int infoc = 1;
		bracketed = false;
		...
		if (g0 >= 0) {
			// We're looking in a direction of positive gradient. This wont'
			// work.
			// set dir = -grad
			for (int i = 0; i < x.length; i++) {
				dir[i] = -grad[i];
		...
		do {
			...
			double stpMin; // = aMin; [cdm: this initialization was always
							// overridden below]
			double stpMax; // = aMax; [cdm: this initialization was always
							// overridden below]
			if (bracketed) {
			...
			// this is the first stage where we look for a point that is lower
			// and
			// increasing

			if (stage1 && newPt[f] <= fTest
					&& newPt[g] >= Math.min(ftol, gtol) * g0) {
				stage1 = false;
			}
			...
			if (stage1 && newPt[f] <= bestPt[f] && newPt[f] > fTest) {
				...
				infoc = getStep(x, dir, newX, f0, g0, newPt, bestPt, endPt,
						stpMin, stpMax);
	...
	private int getStep(double[] x, double[] dir, double[] newX, double f0,
		...
		if (newPt[f] > bestPt[f]) {
			info = 1;
			bound = true;
			theta = 3 * (bestPt[f] - newPt[f]) / (newPt[a] - bestPt[a])
					+ bestPt[g] + newPt[g];
			...
			stpq = bestPt[a]
					+ ((bestPt[g] / ((bestPt[f] - newPt[f])
							/ (newPt[a] - bestPt[a]) + bestPt[g])) / 2)
					* (newPt[a] - bestPt[a]);
			...
			s = Math.max(Math.max(theta, bestPt[g]), newPt[g]);
			gamma = s
					* Math.sqrt(Math.max(0.0, (theta / s) * (theta / s)
		...
		if (bracketed && bound) {
			if (endPt[a] > bestPt[a]) {
				newPt[a] = Math.min(bestPt[a] + p66 * (endPt[a] - bestPt[a]),
						newPt[a]);
			} else {
				newPt[a] = Math.max(bestPt[a] + p66 * (endPt[a] - bestPt[a]),
						newPt[a]);
			}
		}
